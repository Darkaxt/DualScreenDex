...
export function SettingsPage({ catalog, state, send, onUpload, onOpenCapabilities = () => undefined, onOpenMapper = () => undefined }: { catalog: Catalog | null; state: State; send: (type: string, values?: Record<string, string | number | boolean | null>) => void; onUpload: (file: File) => void; onOpenCapabilities?: () => void; onOpenMapper?: () => void }) {
  ...
  return <section class="screen settings-screen">
    ...
    <div class="settings-content" data-scroll-region>
      ...
      <section class="setting-group map-detail-setting"><p class="eyebrow">LOCAL MAP DETAILS</p><label class="range-setting"><span>ICONS <b>{poiPreferences.iconZoomThresholdPercent}%</b></span><input aria-label="Map detail icons" type="range" min="0" max="100" step="5" value={poiPreferences.iconZoomThresholdPercent} onInput={event => updatePoi({ iconZoomThresholdPercent: Number(event.currentTarget.value) })} /></label><label class="range-setting"><span>LABELS <b>{poiPreferences.labelZoomThresholdPercent}%</b></span><input aria-label="Map detail labels" type="range" min={poiPreferences.iconZoomThresholdPercent} max="100" step="5" value={poiPreferences.labelZoomThresholdPercent} onInput={event => updatePoi({ labelZoomThresholdPercent: Number(event.currentTarget.value) })} /></label><p class="setting-note">At 0%, permitted map details are visible at the starting Local view. Higher values reveal them only as you zoom in.</p></section>
