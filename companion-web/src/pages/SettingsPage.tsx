...
export function SettingsPage({
  ...
  return <section class="screen settings-screen">
    <Header title={activeCategory?.label.toUpperCase() ?? 'SETTINGS'} onBack={handleBack} />
    <div ref={contentRef} class="settings-content" data-scroll-region>
      ...
      {category === 'ACCESSIBILITY' && <section class="setting-group"><p class="eyebrow">READABILITY</p><label class="range-setting"><span>FONT SCALE <b>{Math.round(settings.fontScale * 100)}%</b></span><input aria-label="Font scale" type="range" min="0.85" max="1.35" step="0.05" value={settings.fontScale} onInput={event => update({ fontScale: Number(event.currentTarget.value) })} /></label><Segmented values={['AUTO', 'COMFORTABLE', 'COMPACT']} active={settings.density} onSelect={density => update({ density })} label="Density" /><Toggle label="High contrast" checked={settings.highContrast} onChange={highContrast => update({ highContrast })} /></section>}
