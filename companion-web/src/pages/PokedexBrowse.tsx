...
import { formatUiNumber, msg, normalizeRomText } from '../i18n';
...
export function PokedexBrowse({ catalog, state, send, onOpenMap }: { catalog: Catalog; state: State; send: (type: string, values?: Record<string, string | number | boolean | null>) => void; onOpenMap?: () => void }) {
  ...
  const visible = useMemo(() => {
    const normalizedSearch = normalizeRomText(search, state.activeLanguage?.language);
    return catalog.species.filter(species => {
      const status = state.speciesState[species.id];
      ...
      if (normalizedSearch && (organicArea && !identityKnown || (!normalizeRomText(species.name, state.activeLanguage?.language).includes(normalizedSearch) && !String(species.dex).includes(normalizedSearch)))) return false;
  ...
  }, [activeFilter, areaSpeciesIds, catalog.species, policy, search, state.activeLanguage?.language, state.speciesState]);
  ...
  return <section class="screen pokedex-screen">
    ...
    <div class="browse-tools">
      <div class="filter-strip" aria-label={msg('pokedexFilters')}>
        {filters.map(filter => {
          const label = filterLabel(filter);
          return <button key={filter} disabled={!filterEnabled[filter]} aria-pressed={activeFilter === filter} title={!filterEnabled[filter] ? msg('filterUnavailable', label) : undefined} class={activeFilter === filter ? 'active' : ''} onClick={() => send('FILTER', { filter, areaId: null })}>{label}</button>;
        })}
      </div>
    </div>
    <div
    ...
    >
      ...
      >
      {mounted.map(species => { const speciesState = state.speciesState[species.id]; const knowledge = speciesIdentityKnowledge(policy, speciesState); const hidden = knowledge === 'unknown'; const metadataUnlocked = policy === 'DISCOVERED' || Boolean(speciesState?.caught); const types = metadataUnlocked ? uniqueTypeIds(species.typeIds).map(id => catalog.types.find(type => type.id === id)).filter((type): type is Catalog['types'][number] => type != null) : []; return <button key={species.id} class={`species-row ${hidden ? 'identity-hidden' : ''}`} disabled={hidden} aria-label={hidden ? msg('unidentifiedEncounter') : undefined} onClick={() => send('OPEN_SPECIES', { speciesId: species.id })}>
        ...
        {types.length > 0 && <span class="species-row-types" aria-label={msg('types')}>{types.map(type => <TypeChip key={type.id} type={type} />)}</span>}
      ...
      {visible.length === 0 && <div class="empty-state"><strong>{msg('noPokemonFound')}</strong><p>{msg('discoveredPokemonAppear')}</p></div>}
      </div>
    </div>
    {activeFilter !== 'TEAM' && <div class="pokedex-search-dock">
      <div class="pokedex-search-row">
        <label class="search-box"><span>{msg('search')}</span><input aria-label={msg('searchPokemon')} value={search} onInput={event => setSearch(event.currentTarget.value)} placeholder={msg('nameOrNumber')} /></label>
        <output
          class="pokedex-result-count"
          aria-label={msg('pokemonListCount', filterLabel(activeFilter), formatUiNumber(visible.length))}
        >
          {formatUiNumber(visible.length)}
        </output>
      </div>
    </div>}
  </section>;
...
function EncounterWindowMark({ windows }: { windows: Set<EncounterWindow> }) {
  ...
  return day
    ? <svg data-testid="encounter-window-icon" class="encounter-window-icon day" role="img" aria-label={msg('dayEncounter')} viewBox="0 0 24 24">
      <circle cx="12" cy="12" r="4" />
      <path d="M12 2v3M12 19v3M2 12h3M19 12h3M4.9 4.9 7 7M17 17l2.1 2.1M19.1 4.9 17 7M7 17l-2.1 2.1" />
    </svg>
    : <svg data-testid="encounter-window-icon" class="encounter-window-icon night" role="img" aria-label={msg('nightEncounter')} viewBox="0 0 24 24">
