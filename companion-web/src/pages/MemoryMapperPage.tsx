...
export function MemoryMapperPage({ onBack }: { onBack: () => void }) {
  ...
  return <section class="screen mapper-screen">
    ...
    <div class="mapper-content" data-scroll-region>
      <section class="mapper-warning"><strong>RAW MEMORY MAY CONTAIN PRIVATE GAME DATA</strong><p>This lab is isolated from the Pokédex. It sends only RetroArch READ_CORE_MEMORY commands and starts disabled after every app launch.</p></section>
      {!state?.enabled ? <section class="paper-panel mapper-enable"><button class="primary-button" type="button" onClick={enable}>ENABLE FOR THIS SESSION</button></section> : <>
        <section class="mapper-identity"><span><small>CORE</small><strong>{state.coreIdentity ?? 'UNKNOWN'}</strong></span><span><small>CONTENT</small><strong>{state.contentIdentity ?? 'UNKNOWN'}</strong></span><button type="button" onClick={() => void act('DISABLE')}>DISABLE</button></section>
