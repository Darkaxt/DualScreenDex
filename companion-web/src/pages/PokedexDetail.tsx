...
export function PokedexDetail({
  ...
  return <section class="screen detail-screen">
    ...
    <div class="detail-content" data-scroll-region>
      {!unlocked && <div class="paper-panel withheld"><strong>KNOWLEDGE WITHHELD</strong><p>Organic mode unlocks the complete ROM entry after this species is recruited.</p></div>}
      ...
      {unlocked && tab === 'MORE' && <div class="paper-panel more-sections">
        ...
        {species.evolutions.length > 0 && <section><p class="eyebrow">EVOLUTIONS</p>{species.evolutions.map((evolution, index) => <div class="data-row" key={`${evolution.targetSpeciesId}-${index}`}><strong>{evolution.targetName}</strong><span>{evolution.condition}</span></div>)}</section>}
