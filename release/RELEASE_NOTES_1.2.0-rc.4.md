...
DualDex 1.2.0 RC4 packages the complete six-stage localization system. Manual Settings choices provide deterministic interface and ROM-content language selection even when `AUTO` cannot resolve a supported device or live-game language. It supersedes the failed RC1, RC2, and RC3 candidates with all Web, desktop-server, and ordinary CI app-test assertions aligned to the localized presentation contract.
...
- Keep private real-ROM control tests conditional on their explicit local inputs, and validate cache-only item projection with a structurally complete localized POI fixture.
...
- This candidate uses Android version code `1020004`.
