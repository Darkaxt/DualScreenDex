...
> [!IMPORTANT]
> The pure-Kotlin ROM parser, materialized SQLite catalog, Gen I–III SaveRAM readers, validated live-WRAM paths, loopback web host, Thor-first UI, passive RetroArch activation, Docked/Overlay modes, and isolated read-only issue reports are implemented. Stable `v1.0.0` provides the complete v1 baseline. Candidate `v1.1.0-rc.67` retains one unified snapshot as the sole transient-state authority and corrects Pokédex Height Comparison scaling so visible trainer and Pokémon artwork—not transparent image padding—represents the stated physical height. Unsupported features remain explicit instead of aborting otherwise valid catalogs.
>
...
| Signed candidate target | [`v1.1.0-rc.67`](https://github.com/Darkaxt/DualScreenDex/releases/tag/v1.1.0-rc.67) is the latest published prerelease. Protected workflow `32982975585` built and signed commit `b9a650af587d137f905138592eaa9f2dd3d996ba`; the anonymous public APK matches all published checksums, GitHub asset digests, provenance, package identity, and the pinned release certificate. |
