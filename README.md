...
> [!IMPORTANT]
> The pure-Kotlin ROM parser, materialized SQLite catalog, Gen I–III SaveRAM readers, save-backed knowledge, loopback web host, Thor-first UI, passive RetroArch activation, Docked/Overlay modes, and isolated read-only issue reports are implemented. Public prerelease [`v1.0.0-rc.20`](https://github.com/Darkaxt/DualScreenDex/releases/tag/v1.0.0-rc.20) previews the ROM-derived Map First experience for the exact validated Modern Emerald 3.5 ROM, with Organic fog of war, Area Dex navigation, and masked unseen encounter rows. Other ROMs retain the existing Pokédex and Area layout with maps safely disabled. Reviewed ROM behavior is published in [ROM Hacks Compatibility](reports/dualdex-rom-hacks-compatibility.md), with lower-level evidence in [Parser Compatibility](reports/dualdex-parser-compatibility.md). The stopped experimental [THUMB ability-mechanics status](reports/thumb-ability-mechanics-status.md) is published separately and is not part of the APK. Stable `v1.0.0` follows signed-candidate validation on the Thor.
