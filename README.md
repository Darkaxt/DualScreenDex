...
> [!IMPORTANT]
> The pure-Kotlin ROM parser, materialized SQLite catalog, Gen I–III SaveRAM readers, validated live-WRAM paths, loopback web host, Thor-first UI, passive RetroArch activation, Docked/Overlay modes, and isolated read-only issue reports are implemented. Stable `v1.0.0` provides the complete v1 baseline. Candidate `v1.1.0-rc.34` corrects Gen III Local-map sign and entrance naming, merges duplicate nearby sign/door POIs, and removes redundant or unidentified map labels while retaining RC33's Organic discovery and collection tracking. Unsupported features remain explicit instead of aborting otherwise valid catalogs.
...
| Signed candidate target | `v1.1.0-rc.34` is prepared for the protected signing workflow with checksums and provenance. |
