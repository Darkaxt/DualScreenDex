...
> [!IMPORTANT]
> The pure-Kotlin ROM parser, materialized SQLite catalog, Gen I–III SaveRAM readers, validated live-WRAM paths, loopback web host, Thor-first UI, passive RetroArch activation, Docked/Overlay modes, and isolated read-only issue reports are implemented. Stable `v1.0.0` provides the complete v1 baseline. Candidate `v1.1.0-rc.61` adds compiled-authorized expanded split capture balls with per-ROM item relationships and per-entry failure isolation while preserving standard/integrated ball paths and RC60's embedded acquisitions. Schema-41 catalogs intentionally rebuild once to materialize the expanded ball records. Unsupported features remain explicit instead of aborting otherwise valid catalogs.
...
| Signed candidate target | [`v1.1.0-rc.60`](https://github.com/Darkaxt/DualScreenDex/releases/tag/v1.1.0-rc.60) is the latest published prerelease; protected workflow `32784097897` built, signed, verified, and published the exact annotated-tag commit. |
