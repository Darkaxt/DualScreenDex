...
| 2.4 | Local/passive operation and explicitly scoped config edits | SAF setup, loopback host, exact-key config editor | Config installer/editor/session tests | Nightly NCI setup evidence | Verified |
...
| 7 | SAF grants, exact config edits, restart/effective-file verification, no PID/Cocoon dependency | Setup coordinator and `retroarch-session` | Config, restart, session and route suites | Dedicated nightly NCI evidence | Verified |
...
| 9 | Save discovery, heartbeat polling, Gen I–III parsing and gated filters | Save resolver/monitor/readers and knowledge mapper | Save/checksum/corruption/association suites | Named save reports and full Stage 7 rerun | Verified |
