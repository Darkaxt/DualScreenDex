...
Add the live-map paragraph, centered 46% map capture, and `Local map` caption.
...
Add the ROM-catalog/live-knowledge paragraph followed by Browser, Entry text, height comparison, Stats, Moves, Area, ability behavior, and evolutions/locations. Use one centered 46% browser image and paired 46% rows for the remaining captures, with the final evolution image centered at 46%.
...
Add the live-card paragraph and centered 46% approved card capture, then close the tour with the signed-RC66/Modern-Emerald provenance statement.
...
Assert five `###` category headings, 18 unique `docs/images/live/dualdex-rc66-*.webp` references, one visible caption for every asset, a 46% width for every individual image, and no references to the former seven-image gallery block.
