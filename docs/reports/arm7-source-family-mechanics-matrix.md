...
- The returned attack value is not inferred from the modifier callsite. The decoded unsigned-division body preserves the typed Attack derivation through stat-stage arithmetic, `MulModifier` proves the accumulated Q4.12 writeback, and the complete `ApplyModifier` body at `0x4EDD0-0x4EDDE` proves a pure `(modifier * attack + 0x800) >> 12` return. A live-ROM counterfactual replacing that final helper body with an immediate return withholds every Classic tuple.
