...
internal class CompiledGbaFieldMapOwnerFlow private constructor(
    ...
    companion object {
        fun resolve(
            ...
            val end = owner + 0x17A
            if (owner < 0xC0 || owner and 3 != 0 || !r.range(owner, 0x18C)) return null
            ...
            val codeRanges = listOf(
                owner.toLong() until owner.toLong() + 0x18C,
            ...
            val knownEntries = listOf(owner + 0x18C, windowFlow.rendererOffset,
            ...
            if (arms.indices.any { r.word(owner + 0x2C + it * 4) != 0x08000000L + owner + arms[it] }) return null

            cancellation.throwIfCancellationRequested()
            if (!r.ops(owner + 0x48, 0x6820, 0x3008, 0x2100) ||
                !r.externalCall(owner + 0x4E, owner, 0x18C) ||
                !r.ops(owner + 0x52, 0x2000, 0x2100) ||
                !r.externalCall(owner + 0x56, owner, 0x18C) ||
                !r.ops(owner + 0x5A, 0x2001, 0x2101) ||
                !r.externalCall(owner + 0x5E, owner, 0x18C) ||
                !r.ops(owner + 0x62, 0x6821) ||
                r.literal(owner + 0x64, 0, owner + 0x6C) != stateOffset ||
                !r.ops(owner + 0x66, 0x1809) ||
                r.branch(owner + 0x68) != owner + 0x13C
            ) return null
            ...
            if (r.immediate(owner + 0x70, 0) != declaration.window ||
                ...
                r.literal(owner + 0x7C, 2, owner + 0xB8) != 0x08000000L + declaration.source ||
                ...
                !r.externalCall(owner + 0x94, owner, 0x18C) ||
                ...
                !r.externalCall(owner + 0xA4, owner, 0x18C) ||
                !r.ops(owner + 0xA8, 0x2001, 0x4240, 0x9400, 0x2100, 0x2210, 0x2300) ||
                r.branch(owner + 0xB4) != owner + 0x130
            ) return null
            val schedule = r.call(owner + 0x94) ?: return null
            val dynamic = r.call(owner + 0xA4) ?: return null
            if (schedule == dynamic || dynamic != owner + 0x18C ||
                dynamic == declaration.frame || dynamic == declaration.printer
            ) return null

            cancellation.throwIfCancellationRequested()
            if (!r.ops(owner + 0xBC, 0x2182, 0x0149, 0x2000) ||
                !r.externalCall(owner + 0xC2, owner, 0x18C) ||
                !r.ops(owner + 0xC6, 0x2000) ||
                !r.externalCall(owner + 0xC8, owner, 0x18C) ||
                !r.ops(owner + 0xCC, 0x2002) ||
                r.call(owner + 0xCE) != r.call(owner + 0xC8) ||
                r.literal(owner + 0xD2, 0, owner + 0xDC) != handlerRoot ||
                !r.ops(owner + 0xD4, 0x6801) ||
                r.literal(owner + 0xD6, 0, owner + 0xE0) != stateOffset ||
                !r.ops(owner + 0xD8, 0x1809) ||
                r.branch(owner + 0xDA) != owner + 0x13C
            ) return null

            val fadeRoot = r.literal(owner + 0xE4, 0, owner + 0xF4) ?: return null
            if (!ramWord(fadeRoot) || fadeRoot == handlerRoot ||
                !r.ops(owner + 0xE6, 0x79C1, 0x2080, 0x4008, 0x2800) ||
                r.conditionalBranch(owner + 0xEE, 1) != end ||
                !r.ops(owner + 0xF0, 0x6821) ||
                r.branch(owner + 0xF2) != owner + 0x138
            ) return null

            if (!r.externalCall(owner + 0xF8, owner, 0x18C) ||
                !r.ops(owner + 0xFC, 0x0600, 0x0E00, 0x2803) ||
                r.conditionalBranch(owner + 0x102, 1) != owner + 0x10A ||
                r.call(owner + 0x104) != dynamic ||
                r.branch(owner + 0x108) != end ||
                !r.ops(owner + 0x10A, 0x2803) ||
                r.conditionalBranch(owner + 0x10C, 0xB) != end ||
                !r.ops(owner + 0x10E, 0x2805) ||
                r.conditionalBranch(owner + 0x110, 0xC) != end ||
                r.literal(owner + 0x112, 0, owner + 0x11C) != handlerRoot ||
                !r.ops(owner + 0x114, 0x6801) ||
                r.literal(owner + 0x116, 0, owner + 0x120) != stateOffset ||
                !r.ops(owner + 0x118, 0x1809) ||
                r.branch(owner + 0x11A) != owner + 0x13C
            ) return null

            cancellation.throwIfCancellationRequested()
            if (!r.ops(owner + 0x124, 0x2001, 0x4240, 0x2100, 0x9100, 0x2200, 0x2310) ||
                !r.externalCall(owner + 0x130, owner, 0x18C) ||
                r.literal(owner + 0x134, 0, owner + 0x144) != handlerRoot ||
                !r.ops(owner + 0x136, 0x6801) ||
                r.literal(owner + 0x138, 2, owner + 0x148) != stateOffset ||
                !r.ops(owner + 0x13A, 0x1889) ||
                !r.ops(owner + 0x13C, 0x8808, 0x3001, 0x8008) ||
                r.branch(owner + 0x142) != end
            ) return null
            val fade = r.call(owner + 0x130) ?: return null

            if (r.literal(owner + 0x14C, 0, owner + 0x184) != fadeRoot ||
                !r.ops(owner + 0x14E, 0x79C1, 0x2080, 0x4008, 0x0600, 0x0E05, 0x2D00) ||
                r.conditionalBranch(owner + 0x15A, 1) != end ||
                !r.externalCall(owner + 0x15C, owner, 0x18C) ||
                r.literal(owner + 0x160, 4, owner + 0x188) != handlerRoot ||
                !r.ops(owner + 0x162, 0x6820, 0x6800) ||
                !r.externalCall(owner + 0x166, owner, 0x18C) ||
                !r.ops(owner + 0x16A, 0x6820, 0x2800) ||
                r.conditionalBranch(owner + 0x16E, 0) != owner + 0x176 ||
                !r.externalCall(owner + 0x170, owner, 0x18C) ||
                !r.ops(owner + 0x174, 0x6025) ||
                !r.externalCall(owner + 0x176, owner, 0x18C) ||
