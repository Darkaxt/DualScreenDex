...
internal object OfficialLanguageResolver {
    fun headerCandidate(header: RomHeader, generation: Int): OfficialLanguageCandidate? {
        ...
        val language = LANGUAGE_BY_MARKER[marker.uppercaseChar()] ?: return null
        return OfficialLanguageCandidate(
            language = language,
            evidence = LanguageEvidence(
                kind = LanguageEvidenceKind.HEADER_REGION_HINT,
                summary = "recognized official product identifier seeds a ${language.value} candidate",
    ...
    fun preferredProbeCodec(
        ...
        val language = headerCandidate(header, generation)?.language
            ...
            ?: LanguageTag.ENGLISH
        return requireNotNull(LanguageRegistry.candidateCodec(language, generation, header.platform)) {
    ...
    private fun redBlueMenuLanguage(
        ...
        if (generation != 1 || header.platform !in setOf(Platform.GB, Platform.GBC) ||
    ...
    private val GEN2_PRODUCT_PREFIXES = setOf("AAU", "AAX", "BYT")
