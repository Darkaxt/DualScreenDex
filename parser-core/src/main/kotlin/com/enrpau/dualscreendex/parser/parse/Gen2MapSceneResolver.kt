...
internal object Gen2MapSceneResolver {
    data class Source(
        val baseAreaId: Int,
    ...
    fun resolve(
        ...
        for (source in sources.sortedBy(Source::baseAreaId)) {
            ...
            val flagsResult = runCatching {
                require(source.attributes + FIXED_ATTRIBUTES_BYTES <= bankEnd(rom, source.attributesBank))
                rom.u8(source.attributes + CONNECTION_FLAGS_OFFSET).also {
                    require(it and CONNECTION_MASK.inv() == 0)
                }
            }
            ...
            var record = source.attributes + FIXED_ATTRIBUTES_BYTES
    ...
    private fun readConstraint(
        ...
        require(record + CONNECTION_BYTES <= bankEnd(rom, source.attributesBank)) {
            "connection record is truncated"
        }
        val targetId = (rom.u8(record) shl 8) or rom.u8(record + TARGET_MAP_OFFSET)
    ...
    private fun boundedReasons(reasons: List<String>): List<String> = when {
        ...
        else -> reasons.take(MAX_DIAGNOSTICS) +
            "${reasons.size - MAX_DIAGNOSTICS} additional Gen II connection diagnostics omitted"
    }
    ...
    private const val FIXED_ATTRIBUTES_BYTES = 12
    private const val CONNECTION_FLAGS_OFFSET = 11
    private const val CONNECTION_MASK = 0x0f
    private const val CONNECTION_BYTES = 12
    private const val TARGET_MAP_OFFSET = 1
    private const val STRIP_POINTER_OFFSET = 2
    private const val DESTINATION_POINTER_OFFSET = 4
    private const val STRIP_LENGTH_OFFSET = 6
    private const val CONNECTED_WIDTH_OFFSET = 7
    private const val Y_ALIGNMENT_OFFSET = 8
    private const val X_ALIGNMENT_OFFSET = 9
    private const val VIEW_POINTER_OFFSET = 10
